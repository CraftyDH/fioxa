...
impl PageTableManager {
    pub fn map_memory(&mut self, virtual_memory: u64, physical_memory: u64) -> Option<()> {
        ...
        let pd = Self::get_or_create_table(&mut pdp.entries[indexer.pd_i as usize])?;

        let pt = Self::get_or_create_table(&mut pd.entries[indexer.pt_i as usize])?;
    ...
    pub fn unmap_memory(&self, virtual_memory: u64) {
        let indexer = PageMapIndexer::new(virtual_memory);
        let pml4 = unsafe { &mut *(self.page_lvl4_addr as *mut PageTable) };
