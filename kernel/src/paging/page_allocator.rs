...
impl<'mmap, 'bit> PageFrameAllocator<'mmap, 'bit> {
    pub fn new(mmap: &'mmap [MemoryDescriptor]) -> Self {
        ...
        let buf = unsafe {
            let ptr: *mut u8 = (bitmap_start_frame * 4096) as *mut u8;
            // core::ptr::from_exposed_addr(addr)
            core::ptr::write_bytes(ptr, 0xFF, bitmap_pages * 4096);
            &mut *slice_from_raw_parts_mut(ptr, bitmap_pages * 4096)
        };
    ...
    pub fn free_page(&mut self, page: usize) -> Option<()> {
        let bits = self.page_bitmap.view_bits_mut::<bitvec::prelude::Lsb0>();
        let bit = bits.get_mut(page)?;
        if *bit {
            bit.commit(false);
            if page > self.last_page_used {
    ...
    pub fn lock_page(&mut self, page: usize) -> Option<()> {
        let bits = self.page_bitmap.view_bits_mut::<bitvec::prelude::Lsb0>();
        let bit = bits.get_mut(page)?;
        if !*bit {
            bit.commit(true);
