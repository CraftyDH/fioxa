...
define_syscalls! {
    // misc
    raw_sys_echo(val: usize, val2: *mut usize) -> result_t,
    raw_sys_yield() -> result_t,
    raw_sys_sleep(ms: u64, slept: *mut u64) -> result_t,
    raw_sys_exit() -> !,
    ...
    raw_sys_read_args(buffer: *mut u8, len: usize, out_len: *mut usize) -> result_t,
    raw_sys_pid(pid: *mut pid_t) -> result_t,
    raw_sys_log(level: u32, target: *const u8, target_len: usize, message: *const u8, message_len: usize) -> result_t,
    ...
    // channel
    raw_sys_channel_create(left: *mut hid_t, right: *mut hid_t) -> result_t,
    ...
    // interrupt
    raw_sys_interrupt_create(out: *mut hid_t) -> result_t,
    ...
    // port
    raw_sys_port_create(out: *mut hid_t) -> result_t,
    ...
    // process
    raw_sys_process_spawn_thread(func: *const (), arg: *mut (), out: *mut tid_t) -> result_t,
    raw_sys_process_exit_code(handle: hid_t, exit: *mut usize) -> result_t,

    // message
    raw_sys_message_create(data: *const u8, data_len: usize, out: *mut hid_t) -> result_t,
    ...
    // vmo
    raw_sys_vmo_mmap_create(base: *mut (), length: usize, out: *mut hid_t) -> result_t,
    raw_sys_vmo_anonymous_create(length: usize, flags: u32, out: *mut hid_t) -> result_t,
